  ...
  if (collapsible === "none") {
    return (
      <div
        data-slot="sidebar"
        className={cn(
          "bg-sidebar/95 backdrop-blur-xl text-sidebar-foreground flex h-full w-(--sidebar-width) flex-col",
          className
        )}
        {...props}
      >
    ...
    >
      ...
      >
        <div
          ...
          className="bg-sidebar group-data-[variant=floating]:rounded-xl group-data-[variant=floating]:shadow-lg group-data-[variant=floating]:border group-data-[variant=floating]:border-sidebar-border/50 flex size-full flex-col"
        >
          {children}
        </div>
      </div>
...
function SidebarInput({
  ...
  return (
    <Input
      ...
      data-sidebar="input"
      className={cn("bg-background/50 focus:bg-background border-transparent focus:border-input shadow-none h-8 w-full transition-colors", className)}
      {...props}
    />
  )
}
...
function SidebarHeader({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      data-sidebar="header"
      className={cn("gap-1 p-3 flex flex-col", className)}
      {...props}
    />
  )
}
...
function SidebarFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      data-sidebar="footer"
      className={cn("gap-1 p-3 flex flex-col w-full", className)}
      {...props}
    />
  )
}
...
function SidebarSeparator({
  ...
  return (
    <Separator
      ...
      className={cn("bg-sidebar-border/20 mx-3 w-auto", className)}
      {...props}
    />
  )
}
...
function SidebarContent({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "no-scrollbar gap-0 flex min-h-0 flex-1 flex-col overflow-auto group-data-[collapsible=icon]:overflow-hidden py-1",
        className
      )}
      {...props}
    />
...
function SidebarGroup({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      data-sidebar="group"
      className={cn(
        "px-2 py-0.5 relative flex w-full min-w-0 flex-col",
        className
      )}
      {...props}
    />
...
function SidebarGroupLabel({
  ...
  return (
    <Comp
      ...
      className={cn(
        "text-sidebar-foreground/60 ring-sidebar-ring h-7 rounded-sm px-2 text-xs font-medium transition-[margin,opacity] duration-200 ease-linear group-data-[collapsible=icon]:-mt-8 group-data-[collapsible=icon]:opacity-0 focus-visible:ring-2 [&>svg]:size-4 flex shrink-0 items-center outline-hidden [&>svg]:shrink-0",
        className
      )}
      {...props}
    />
...
const sidebarMenuButtonVariants = cva(
  "ring-sidebar-ring hover:bg-sidebar-accent/40 active:bg-sidebar-accent/60 data-[active=true]:bg-sidebar-accent/50 data-[active=true]:text-sidebar-accent-foreground data-[active=true]:font-medium data-open:hover:bg-sidebar-accent/40 gap-2.5 rounded-lg p-2 text-left text-sm transition-[width,height,padding,background-color] duration-200 group-has-data-[sidebar=menu-action]/menu-item:pr-8 group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! focus-visible:ring-2 peer/menu-button flex w-full items-center overflow-hidden outline-hidden disabled:pointer-events-none disabled:opacity-50 aria-disabled:pointer-events-none aria-disabled:opacity-50 [&>span:last-child]:truncate [&_svg]:size-4 [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        default: "hover:bg-sidebar-accent/40 text-sidebar-foreground/80 hover:text-sidebar-foreground",
        outline: "bg-background hover:bg-sidebar-accent/40 shadow-[0_0_0_1px_hsl(var(--sidebar-border))] hover:shadow-[0_0_0_1px_hsl(var(--sidebar-border))]",
      },
      size: {
        default: "h-9 text-sm",
        sm: "h-8 text-sm",
